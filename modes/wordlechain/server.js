...
function isDoubleConsonantStart(seq) {
    if (seq.length < 2) return false;
    return !VOWELS.has(seq[0]) && !VOWELS.has(seq[1]);
}

/**
 * After a valid word, pick 1–4 ending letters as the next prefix.
 * Probabilities: 50% → 1, 25% → 2, 15% → 3, 10% → 4
 * Skips sequences that start with double consonants.
 */
function generateNextPrefix(word, language = 'it') {
    ...
    let counts;
    if (roll < 50) counts = [1];
    else if (roll < 75) counts = [2, 1];
    else if (roll < 90) counts = [3, 2, 1];
    else counts = [4, 3, 2, 1];

    for (const n of counts) {
        if (n > upper.length) continue;
        const seq = upper.slice(-n);
        if (!isDoubleConsonantStart(seq)) {
            if (wordManager.hasWordWithPrefix(seq, language)) {
                return seq;
            }
        }
    }
    // Fallback: try just the last letter if multiple failed
    const lastLetter = upper.slice(-1);
    if (wordManager.hasWordWithPrefix(lastLetter, language)) {
        return lastLetter;
    }
    // Extreme fallback: a random vowel
